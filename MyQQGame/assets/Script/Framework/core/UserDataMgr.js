...
var UserDataMgr = cc.Class({
		...
		if(StorageData == null || StorageData == "")
		{
			...
			for (var k in ChapterData[1].cities){
				...
				for (var i = 1; i <= ChapterData[1].cities[k].missionNum; i++){
					...
					missionId = ChapterData[1].countryId + ChapterData[1].cities[k].cityId + id;
					var isOpen = k == 1 && i == 1;
					cityData.mission[i] = {
						index : i,
						missionId : missionId,
	...
	UpdateuserData: function(missionId, star){
		for (var k1 in this.userData){
			for (var k2 in this.userData[k1].mission){
				if (this.userData[k1].mission[k2].missionId == missionId){
					if (this.userData[k1].mission[k2].star == 0 && star > 0){
						var nextData = this.userData[k1].mission[k2 + 1];
						if (nextData){
							///下一关开启
							this.userData[k1].mission[k2 + 1].isOpen = true;
						}
						else
						{
							///下一城市开启
							this.userData[k1 + 1].isOpen = true;
							this.userData[k1 + 1].mission[1].isOpen = true;
						}
					}
					///如果通关星数大于记录则修改星星数
					if (star > this.userData[k1].mission[k2].star){
						///更新城市总星星数
						this.userData[k1].star += star - this.userData[k1].mission[k2].star;
						///更新关卡星星数
						this.userData[k1].mission[k2].star = star
					}
					break;
				}
			}
